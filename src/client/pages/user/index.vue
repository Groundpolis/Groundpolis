...
<div class="mk-user-page" v-if="user" v-size="{ max: [500] }">
	...
	<template v-if="$route.name == 'user'">
		<div class="pins">
			<x-note v-for="note in user.pinnedNotes" class="note" :note="note" @updated="pinnedNoteUpdated(note, $event)" :key="note.id" :pinned="true"/>
		</div>
...
export default Vue.extend({
	...
	data() {
		return {
			...
			faExclamationTriangle, faEllipsisH, faRobot, faLock, faBookmark, farBookmark, faChartBar, faImage, faBirthdayCake, faMapMarker, faCalendarAlt, faMars, faVenus, faGenderless
		};
	},
