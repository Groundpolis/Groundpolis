...
export function getStream(): Stream { 
	if (stream === null) throw new Error('Stream is not initialized!');
	return stream;
}

export function initializeStream(conf: StreamConfig, forceRecreate = false) { 
	if (!forceRecreate && stream !== null) throw new Error('Stream is already initialized!');
